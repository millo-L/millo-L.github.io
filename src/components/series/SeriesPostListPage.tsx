    ...
    return (
        <MainTemplate>
            <SEO
                ...
                }/${allMarkdownRemark.edges[0].node.frontmatter.series.replace(
                    / /gi,
                    "-"
                )}`}
                ...
                description={description(
                    allMarkdownRemark.edges[0].node.frontmatter.series
                )}
            />
            <Header
                lang={allMarkdownRemark.edges[0].node.frontmatter.lang}
            ...
            <MainResponsive>
                <SeriesPostList
                    posts={filterByLanguage(
                        reshapePost(allMarkdownRemark),
                        allMarkdownRemark.edges[0].node.frontmatter.lang
                    )}
...
export const pageQuery = graphql`
    query($series: String!) {
        allMarkdownRemark(
            ...
            sort: { fields: frontmatter___released_at, order: ASC }
        ) {
            edges {
                node {
                    id
